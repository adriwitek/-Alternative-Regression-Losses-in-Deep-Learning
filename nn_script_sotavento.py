...
DATASETS_TO_USE = [     'sotavento',
                ]

DATASETS_TARGET_NAMES = {   'sotavento' : 'targ',
                        }


# Must specify min and max value
DATASETS_CLIP_RANGE = {     'sotavento' : (0., 1.),
                        }
...
def sotavento_data_train(dataset_path = DATASETS_PATH):
    
    sota2016_path = "data_target_stv_2016.csv"
    sota2017_path = "data_target_stv_2017.csv"

    if(dataset_path):
        sota2016_path = os.path.join(dataset_path, sota2016_path)
        sota2017_path = os.path.join(dataset_path, sota2017_path)


    sota2016 = pd.read_csv(sota2016_path, sep = ',', index_col=0, parse_dates=True)
    sota2017 = pd.read_csv(sota2017_path, sep = ',', index_col=0, parse_dates=True)
    train = pd.concat([sota2016,sota2017])
    x = select_desired_variables(train.drop(columns = ['targ'])).to_numpy()
    y = train[['targ']].to_numpy()
    return x,y
...
def train_nn(   x,
    ...
    if(SHUFFLE):
        kf = KFold(n_folds, shuffle=SHUFFLE, random_state = seed)
    else:
...
def plot_preddispersion_and_errdistribution(y, 
                                            cv_y_pred_mean, 
                                            target_name,
                                            save_path,
                                            save_report,
    ...
    if(save_report):
        fig_name = 'Real_vs_Predicted_MAE_Values'  + add_tofilename
        if(clipped):
    ...
    if(save_report):
        fig_name = 'MAE_ERROR_DISTRIBUTION'  + add_tofilename
        if(clipped):
            fig_name =  fig_name + '_clipped'
        plt.savefig(os.path.join(save_path,fig_name))
    else:
...
def plot_CV_model_evaluation(   x,
    ...
    print('Getting test data... ', file = file)
    x_test , y_test = sotavento_data_test()

    print('\t Done!', file = file)

    print('REFITING MODEL WITH OPTIMUM HYPERPARAMETER CONFIGURARION ' + str(n_cross_val_predict_execs) + ' times...', file = file)
    # Test data
    cv_predict_mae = None
    cv_predict_maes = []
    l_cv_y_pred = []
    ...
    # Train data
    l_cv_y_pred_traindata = []
    l_cv_y_pred_clipped_traindata = []
    ...
    for i in range(n_cross_val_predict_execs):
        ...
        print('\t\t\t --> Starting Training in iteration:' + str(i+1), file = file)
        # Weights reset
        model_i = clone(model)
        model_i.regressor.named_steps['mlp'].random_state = random_states[i]
        print('\t\t\t\t\t\t Current Model description' + str(model_i), file = file)
        
        t0_i = time.time()
        model_i.fit(x,y)
        t1_i = time.time()
        print(f'\t\t\t\t --> Training Completed in: {( (t1_i - t0_i)/60. ):4f} seconds.',  file = file)
        

        ### TEST DATA
        t0_i = time.time()
        cv_y_pred = model_i.predict(x_test)
        t1_i = time.time()
        print(f'\t\t\t\t --> Predict(Test Data) Completed in: {( (t1_i - t0_i)/60. ):4f} seconds.',  file = file)
        cv_y_pred = cv_y_pred.ravel()
        ...
        cv_predict_mae = mean_absolute_error(y_test.ravel() , cv_y_pred)
        ...
        ### TRAIN DATA
        t0_i = time.time()
        cv_y_pred_traindata = model_i.predict(x)
        t1_i = time.time()
        print(f'\t\t\t\t --> Predict(Train Data) Completed in: {( (t1_i - t0_i)/60. ):4f} seconds.',  file = file)
        cv_y_pred_traindata = cv_y_pred_traindata.ravel()
        l_cv_y_pred_traindata.append(cv_y_pred_traindata)

        if(CLIP_PREDICTIONS):
            l_cv_y_pred_clipped_traindata.append(np.clip(cv_y_pred_traindata, min_clip,max_clip))



    t_1 = time.time() 
    time_spent =  (t_1 - t_0)/60.
    print(f'process completed in {time_spent:4f} minutes.\n', file = file)
    ...
    a_cv_y_pred_traindata = np.array(l_cv_y_pred_traindata).T
    cv_y_pred_mean_traindata = a_cv_y_pred_traindata.mean(axis=1)
    cv_y_pred_median_traindata = np.median(a_cv_y_pred_traindata, axis=1)


    #Model CLIPPED Predictions (train data)
    if(CLIP_PREDICTIONS):
        ...
        a_cv_y_pred_clipped_traindata = np.array(l_cv_y_pred_clipped_traindata).T
        cv_y_pred_mean_clipped_traindata = a_cv_y_pred_clipped_traindata.mean(axis=1)
        cv_y_pred_median_clipped_traindata = np.median(a_cv_y_pred_clipped_traindata, axis=1)
    ...
    # Model Scores
    ### TEST DATA
    print("-------------------USING TEST DATA!------------------------- " ,file = file)
    model_mean = mean_absolute_error(y_test.ravel() , cv_y_pred_mean)
    model_median = mean_absolute_error(y_test.ravel(), cv_y_pred_median)

    if(CLIP_PREDICTIONS):
        model_mean_clipped = mean_absolute_error(y_test.ravel() , cv_y_pred_mean_clipped)
        model_median_clipped = mean_absolute_error(y_test.ravel(), cv_y_pred_median_clipped)
    ...
    ### TRAIN DATA
    print("-------------------USING TRAIN DATA!------------------------- " ,file = file)
    model_mean_traindata = mean_absolute_error(y.ravel() , cv_y_pred_mean_traindata)
    model_median_traindata = mean_absolute_error(y.ravel(), cv_y_pred_median_traindata)

    if(CLIP_PREDICTIONS):
        model_mean_clipped_traindata = mean_absolute_error(y.ravel() , cv_y_pred_mean_clipped_traindata)
        model_median_clipped_traindata = mean_absolute_error(y.ravel(), cv_y_pred_median_clipped_traindata)


    print("MODEL SCORE(TRAIN DATA):" ,file = file)
    print("\t\t ---> MEAN MAE: %.10f" % model_mean_traindata , file = file)
    print("\t\t ---> MEDIAN MAE: %.10f" % model_median_traindata , file = file)
    print("\n\n\n" , file = file)

    if(CLIP_PREDICTIONS):
        print("MODEL SCORE OF CLIPPED PREDICTIONS(TRAIN DATA):" ,file = file)
        print("\t Clipping Interval (min, max):   (" + str(min_clip)  + ' , '+ str(max_clip) + ')' , file = file)
        print("\t\t ---> MEAN MAE: %.10f" % model_mean_clipped_traindata , file = file)
        print("\t\t ---> MEDIAN MAE: %.10f" % model_median_clipped_traindata , file = file)
        print("\n\n\n" , file = file)
    ...
    if(debug):
        ...
        print('-------------------------------------------------------------', file = file)
        print("\n All predictions generated of all .predict executions (l_cv_y_pred) (TEST DATA): \n" ,file = file)
        print(l_cv_y_pred ,file = file)
        print('-------------------------------------------------------------', file = file)
        print('-------------------------------------------------------------', file = file)
        print("\n All predictions generated of all .predict executions (l_cv_y_pred_traindata) (TRAIN DATA): \n" ,file = file)
        print(l_cv_y_pred_traindata ,file = file)
    ...
    if('Quantile' in str(model.regressor_.named_steps['mlp'].loss) ):
        print('\t\t\t QUANTIL LOSS! NOTHING TO DO WITH SOTAVENTO YET', file = file)
        ...
        # For Test Data
        plot_preddispersion_and_errdistribution(y_test.ravel() , cv_y_pred_mean, target_name,save_path,save_report,bins,
                                                scores_005 = None , scores_095 = None,
                                                clipped = False,add_tofilename = '_testdata_')  
        plot_preddispersion_and_errdistribution(y_test.ravel() , cv_y_pred_mean_clipped, target_name,save_path,save_report,bins,
                                                scores_005 = None , scores_095 = None,
                                                clipped = True,add_tofilename = '_testdata_')  




        # For Train Data
        plot_preddispersion_and_errdistribution(y.ravel() , cv_y_pred_mean_traindata, target_name,save_path,save_report,bins,
                                                scores_005 = None , scores_095 = None,
                                                clipped = False,add_tofilename = '_traindata_')  
        plot_preddispersion_and_errdistribution(y.ravel() , cv_y_pred_mean_clipped_traindata, target_name,save_path,save_report,bins,
                                                scores_005 = None , scores_095 = None,
                                                clipped = True,add_tofilename = '_traindata_') 
...
def plot_cv_search_complete_report(     x, 
    ...
    #thins will be saved in SAVE_FOLDER
    if(save_report):
        if(save_path):
            if(save_name):
...
def main( argv):
        ...
        update_log_status( debug= debug, report_folder_name = save_path, message = 'STARTING REPORT... \t\t(Sotavento Script Version)' )
        ...
        for dataset_name_i in datasets:
                ...
                '''
                x,y =  sotavento_data_train()
                target_name = DATASETS_TARGET_NAMES['sotavento'] 
